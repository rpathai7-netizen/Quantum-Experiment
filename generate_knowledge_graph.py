...
def create_quantum_knowledge_base(project_root, output_dir):
    ...
    test_files = (
        list((project_root / "tests").glob("test_*.py"))
        if (project_root / "tests").exists()
        else []
    )
    all_files = [f for f in python_files + test_files if f.exists()][
        :15
    ]  # Limit to 15 files for processing
            ...
            graph_data["nodes"].append(
                {
                    ...
                    "category": (
                        "quantum_class"
                        if any(
                            x in cls_name for x in ["Quantum", "Circuit", "Simulator"]
                        )
                        else "class"
                    ),
                }
    ...
    print(
        f"✅ Generated graph.json: {len(graph_data['nodes'])} nodes, {len(graph_data['edges'])} edges"
    )
    ...
    with open(report_path, "w") as f:
        ...
        for module_name, module_info in sorted(modules.items()):
            ...
            if module_info["imports"]:
                f.write(
                    f"- **Dependencies**: {', '.join(set(module_info['imports'][:3]))}\n"
                )
            f.write("\n")
        ...
        quantum_classes = [
            ...
            if any(x in name for x in ["Quantum", "Circuit", "Simulator", "Analyzer"])
        ]
